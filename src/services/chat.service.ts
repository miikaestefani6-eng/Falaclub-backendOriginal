...
export async function processChatMessage({
  ...
  const { data: savedUserMessage, error: userMessageError } = await supabaseAdmin
    .from('messages')
    .insert({ conversation_id: activeConversationId, sender: 'user', content: message })
    .select('id')
    .single();
  ...
  const chatHistory: Message[] = (recentMessages ?? [])
    ...
    .slice(0, 20)
    .reverse()
    .filter((item): item is typeof item & { sender: 'user' | 'assistant' } => item.sender === 'user' || item.sender === 'assistant')
  ...
  const { data: savedMiaMessage, error: miaMessageError } = await supabaseAdmin
    .from('messages')
    .insert({ conversation_id: activeConversationId, sender: 'assistant', content: reply })
    .select('id')
    .single();
