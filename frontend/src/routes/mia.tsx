...
import { mensagensMia } from "@/lib/mock-data";
...
type PerfilAluno = {
  ...
  xp_total?: number;
  streak_count?: number;
  recent_activities?: Array<{ activity_type?: string; skill?: string; language?: string; level?: string; minutes?: number; xp_earned?: number; completed_at?: string; metadata?: Record<string, unknown> | null }>;
};

function MiaChat() {
  const navigate = useNavigate();
  const [mensagens, setMensagens] = useState<ChatMessage[]>(mensagensMia as ChatMessage[]);
  ...
  function tocarAudio(audioData: string) { const src = audioData.startsWith("http") ? audioData : `data:audio/mpeg;base64,${audioData}`; new Audio(src).play().catch((error) => console.error("Erro ao reproduzir áudio:", error)); }
  async function enviar(valor: string) {
    ...
    try { const result = await sendMiaMessage(conteudo, conversationId); setConversationId(result.conversationId); setMensagens((m) => [...m, { de: "mia", texto: result.reply }]); void concederXP(10); }
    catch (error) { console.error("Erro ao enviar mensagem para a Mia:", error); setErro(error instanceof Error ? error.message : "Não consegui falar com a Mia agora."); setMensagens((m) => [...m, { de: "mia", texto: "Tive um probleminha para responder agora. Tenta novamente em instantes? ☕" }]); }
  ...
  async function enviarAudioParaBackend(blob: Blob) {
    ...
    try { const result = await sendMiaVoice(blob, conversationId); setConversationId(result.conversationId); setMensagens((m) => { const copia = [...m]; const ultimo = copia[copia.length - 1]; if (ultimo?.de === "aluno" && ultimo.texto === "🎤 Processando seu áudio...") copia[copia.length - 1] = { de: "aluno", texto: result.userTranscript }; return [...copia, { de: "mia", texto: result.replyText, audio: result.audioBase64 }]; }); if (result.audioBase64) tocarAudio(result.audioBase64); void concederXP(10); }
  ...
  async function startRecording() {
    ...
    try { const stream = await navigator.mediaDevices.getUserMedia({ audio: true }); const recorder = new MediaRecorder(stream); mediaRecorderRef.current = recorder; audioChunksRef.current = []; recorder.ondataavailable = (event) => { if (event.data.size > 0) audioChunksRef.current.push(event.data); }; recorder.onstop = async () => { const blob = new Blob(audioChunksRef.current, { type: recorder.mimeType || "audio/webm" }); stream.getTracks().forEach((track) => track.stop()); await enviarAudioParaBackend(blob); }; recorder.start(); setIsRecording(true); }
