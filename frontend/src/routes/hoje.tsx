...
export const Route = createFileRoute("/hoje")({
  head: () => ({ meta: [{ title: "Hoje — FalaClub" }, { name: "description", content: "Seu plano do dia: XP, sequência, atividades, flashcards e Drops." }] }),
  component: Hoje,
});
...
function Hoje() {
  ...
  const planoDoDia = useMemo(() => {
    const habilidadesFeitas = new Set(atividadesHoje.map((atividade) => atividade.skill?.trim().toLowerCase()).filter(Boolean));
    const fezFala = ["fala", "speaking", "speaking_practice"].some((skill) => habilidadesFeitas.has(skill));
    return [
      { id: "mia", titulo: "Conversa com a Mia", tipo: "Fala", min: Math.max(5, Math.round(alunoData.metaMinutos / 2)), feito: fezFala, to: "/mia" as const },
      { id: "flashcards", titulo: revisoes > 0 ? `Revisar ${revisoes} flashcard${revisoes === 1 ? "" : "s"}` : "Flashcards em dia", tipo: "Vocabulário", min: 5, feito: revisoes === 0, to: "/flashcards" as const },
      { id: "drops", titulo: dropDoDia ? "Explorar o Drop de hoje" : "Explorar Drops do idioma", tipo: "Cultura", min: 3, feito: false, to: "/drops" as const },
    ];
  }, [alunoData.metaMinutos, atividadesHoje, dropDoDia, revisoes]);

  const recomendacaoMia = alunoData.minutosHoje >= alunoData.metaMinutos ? "Meta do dia concluída! Se quiser, podemos fazer uma conversa livre. ☕" : `Faltam ${Math.max(0, alunoData.metaMinutos - alunoData.minutosHoje)} minutos para sua meta de hoje. Eu pratico com você. ☕`;
  ...
  return <AppShell titulo={loading ? "Carregando..." : `Fala, ${alunoData.nome}!`} subtitulo={`${alunoData.idioma} ${alunoData.bandeira} · nível ${alunoData.nivel}`}>
    ...
    <div className="grid gap-5 lg:grid-cols-3"><div className="space-y-5 lg:col-span-2">
      ...
      <SectionCard titulo="Prática recomendada" descricao="Seu próximo passo para hoje"><div className="flex items-center justify-between gap-4 rounded-2xl bg-secondary p-4"><div><p className="text-sm font-semibold">Conversa guiada com a Mia</p><p className="mt-1 text-xs text-muted-foreground">Treine {alunoData.idioma} por {Math.max(5, alunoData.metaMinutos - alunoData.minutosHoje)} minutos sobre um tema do seu interesse.</p></div><Link to="/mia" className="shrink-0 rounded-full bg-gradient-brand px-4 py-2 text-sm font-semibold text-primary-foreground">Começar</Link></div></SectionCard>
      <SectionCard titulo="Plano essencial de hoje" descricao={alunoData.objetivo ? `Baseado na sua meta: ${alunoData.objetivo}` : "Baseado no seu perfil e na sua atividade real"}><ul className="space-y-3">{planoDoDia.map(item => <li key={item.id}><Link to={item.to} className="flex items-center gap-3 rounded-2xl border border-border p-3 transition-colors hover:bg-secondary/60"><span className={item.feito ? "flex size-8 items-center justify-center rounded-full bg-success/15 text-success" : "flex size-8 items-center justify-center rounded-full bg-secondary text-secondary-foreground"}>{item.feito ? <Check className="size-4" /> : <Play className="size-4" />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{item.titulo}</p><p className="text-xs text-muted-foreground">{item.tipo} · {item.min} min</p></div></Link></li>)}</ul></SectionCard>
