...
function Hoje() {
  ...
  const nextItem = todayPlan.items.find((item) => !isPlanItemDone(item.skillKey));
  ...
  return <AppShell titulo={loading ? "Carregando..." : `Fala, ${alunoData.nome}!`} subtitulo={`${alunoData.idioma} ${alunoData.bandeira} · nível ${alunoData.nivel}`}>
    ...
    <div className="grid gap-5 lg:grid-cols-3"><div className="space-y-5 lg:col-span-2">
      ...
      <SectionCard titulo="Prática recomendada" descricao={`Hoje: ${todayPlan.focus}`}>{nextItem ? <div className="flex items-center justify-between gap-4 rounded-2xl bg-secondary p-4"><div><p className="text-sm font-semibold">{nextItem.title}</p><p className="mt-1 text-xs text-muted-foreground">{nextItem.type} · {nextItem.minutes} min · alinhado ao seu nível e objetivo.</p></div><Link to={nextItem.to} className="shrink-0 rounded-full bg-gradient-brand px-4 py-2 text-sm font-semibold text-primary-foreground">Começar</Link></div> : <div className="rounded-2xl bg-success/10 p-4"><p className="text-sm font-semibold text-success">Plano do dia concluído ✓</p><p className="mt-1 text-xs text-muted-foreground">Suas práticas concluídas já foram registradas automaticamente no progresso de hoje.</p></div>}</SectionCard>

      <SectionCard titulo="Seu plano de hoje" descricao={alunoData.objetivo ? `Baseado na sua meta: ${alunoData.objetivo}` : "Baseado no seu nível e no tempo disponível"}><ul className="space-y-3">{todayPlan.items.map(item => { const feito = isPlanItemDone(item.skillKey); return <li key={item.id} className="rounded-2xl border border-border p-3"><div className="flex items-center gap-3"><span className={feito ? "flex size-8 items-center justify-center rounded-full bg-success/15 text-success" : "flex size-8 items-center justify-center rounded-full bg-secondary text-secondary-foreground"}>{feito ? <Check className="size-4" /> : <Play className="size-4" />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{item.title}</p><p className="text-xs text-muted-foreground">{item.type} · {item.minutes} min</p></div><Link to={item.to} className="rounded-full bg-secondary px-3 py-1.5 text-xs font-semibold text-secondary-foreground">{feito ? "Rever" : "Abrir"}</Link></div><div className="mt-3 flex justify-end"><span className={feito ? "rounded-full bg-success/15 px-3 py-1.5 text-xs font-semibold text-success" : "rounded-full bg-secondary px-3 py-1.5 text-xs font-semibold text-muted-foreground"}>{feito ? "Concluído" : "Conclua pela atividade"}</span></div></li>; })}</ul></SectionCard>
